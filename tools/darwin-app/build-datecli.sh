#!/bin/bash
# build-datecli.sh — M17: build datecli, a normal Objective-C Foundation program
# exercising NSDateFormatter + NSCalendar + NSDateComponents (date parsing/
# formatting + calendar arithmetic), and stage it at the guest /usr/bin/datecli.
#
...
# 2) Install at the guest path /usr/bin/datecli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/datecli"
    chmod +x "$rootdir/usr/bin/datecli"
    echo "installed: $rootdir/usr/bin/datecli"
}
...
echo ""
echo "=== datecli installed at guest /usr/bin/datecli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/datecli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

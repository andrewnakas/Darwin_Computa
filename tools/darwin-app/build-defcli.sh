#!/bin/bash
# build-defcli.sh — M58: build defcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/defcli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/defcli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/defcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/defcli"
    chmod +x "$rootdir/usr/bin/defcli"
    echo "installed: $rootdir/usr/bin/defcli"
}
...
echo ""
echo "=== defcli installed at guest /usr/bin/defcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/defcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

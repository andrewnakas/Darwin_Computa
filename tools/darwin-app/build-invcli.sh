#!/bin/bash
# build-invcli.sh — M58: build invcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/invcli.
#
...
# 2) Install at the guest path /usr/bin/invcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/invcli"
    chmod +x "$rootdir/usr/bin/invcli"
    echo "installed: $rootdir/usr/bin/invcli"
}
...
echo ""
echo "=== invcli installed at guest /usr/bin/invcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/invcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

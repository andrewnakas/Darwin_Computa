#!/bin/bash
# build-caldiffcli.sh — M41: build caldiffcli, a normal Objective-C Foundation program
# exercising NSCalendar date-from-components + date-difference (inverse of M17), and
# stage it at the guest /usr/bin/caldiffcli.
#
...
# 2) Install at the guest path /usr/bin/caldiffcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/caldiffcli"
    chmod +x "$rootdir/usr/bin/caldiffcli"
    echo "installed: $rootdir/usr/bin/caldiffcli"
}
...
echo ""
echo "=== caldiffcli installed at guest /usr/bin/caldiffcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/caldiffcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

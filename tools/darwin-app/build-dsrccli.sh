#!/bin/bash
# build-dsrccli.sh — M59: build dsrccli, a normal Objective-C Foundation program
# exercising GCD dispatch_source event sources (timer + data source), and
# stage it at the guest /usr/bin/dsrccli.
#
...
# 2) Install at the guest path /usr/bin/dsrccli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/dsrccli"
    chmod +x "$rootdir/usr/bin/dsrccli"
    echo "installed: $rootdir/usr/bin/dsrccli"
}
...
echo ""
echo "=== dsrccli installed at guest /usr/bin/dsrccli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/dsrccli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-undocli.sh — M58: build undocli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/undocli.
#
...
# 2) Install at the guest path /usr/bin/undocli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/undocli"
    chmod +x "$rootdir/usr/bin/undocli"
    echo "installed: $rootdir/usr/bin/undocli"
}
...
echo ""
echo "=== undocli installed at guest /usr/bin/undocli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/undocli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

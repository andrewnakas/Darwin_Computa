#!/bin/bash
# build-pipecli2.sh — M58: build pipecli2, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/pipecli2.
#
...
# 2) Install at the guest path /usr/bin/pipecli2 in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/pipecli2"
    chmod +x "$rootdir/usr/bin/pipecli2"
    echo "installed: $rootdir/usr/bin/pipecli2"
}
...
echo ""
echo "=== pipecli2 installed at guest /usr/bin/pipecli2 ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/pipecli2 bash tools/run_darling_cli.sh /usr/bin/darlingserver"

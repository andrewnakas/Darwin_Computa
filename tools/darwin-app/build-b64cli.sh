#!/bin/bash
# build-b64cli.sh — M22: build b64cli, a normal Objective-C Foundation program
# exercising Base64 (NSData base64) + URL percent-encoding (NSCharacterSet), and
# stage it at the guest /usr/bin/b64cli.
#
...
# 2) Install at the guest path /usr/bin/b64cli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/b64cli"
    chmod +x "$rootdir/usr/bin/b64cli"
    echo "installed: $rootdir/usr/bin/b64cli"
}
...
echo ""
echo "=== b64cli installed at guest /usr/bin/b64cli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/b64cli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

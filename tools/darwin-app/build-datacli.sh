#!/bin/bash
# build-datacli.sh — M30: build datacli, a normal Objective-C Foundation program
# exercising NSData/NSMutableData binary ops (build/slice/search/patch/read), and
# stage it at the guest /usr/bin/datacli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/datacli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/datacli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/datacli"
    chmod +x "$rootdir/usr/bin/datacli"
    echo "installed: $rootdir/usr/bin/datacli"
}
...
echo ""
echo "=== datacli installed at guest /usr/bin/datacli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/datacli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

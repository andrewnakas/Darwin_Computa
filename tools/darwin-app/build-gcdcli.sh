#!/bin/bash
# build-gcdcli.sh — M55: build gcdcli, a normal Objective-C Foundation program
# exercising Grand Central Dispatch (libdispatch) C API, and
# stage it at the guest /usr/bin/gcdcli.
#
...
# 2) Install at the guest path /usr/bin/gcdcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/gcdcli"
    chmod +x "$rootdir/usr/bin/gcdcli"
    echo "installed: $rootdir/usr/bin/gcdcli"
}
...
echo ""
echo "=== gcdcli installed at guest /usr/bin/gcdcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/gcdcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

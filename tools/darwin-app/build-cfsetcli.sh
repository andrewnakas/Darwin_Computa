#!/bin/bash
# build-cfsetcli.sh — M60: build cfsetcli, a pure-C CoreFoundation program
# exercising CFSet + CFBag via the CoreFoundation C API (pure-C, no #import), and
# stage it at the guest /usr/bin/cfsetcli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/cfsetcli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/cfsetcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/cfsetcli"
    chmod +x "$rootdir/usr/bin/cfsetcli"
    echo "installed: $rootdir/usr/bin/cfsetcli"
}
...
echo ""
echo "=== cfsetcli installed at guest /usr/bin/cfsetcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/cfsetcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

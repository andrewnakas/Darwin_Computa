#!/bin/bash
# build-setcli.sh — M34: build setcli, a normal Objective-C Foundation program
# exercising NSSet set algebra + NSOrderedSet ordered-unique collections, and
# stage it at the guest /usr/bin/setcli.
#
...
# 2) Install at the guest path /usr/bin/setcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/setcli"
    chmod +x "$rootdir/usr/bin/setcli"
    echo "installed: $rootdir/usr/bin/setcli"
}
...
echo ""
echo "=== setcli installed at guest /usr/bin/setcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/setcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

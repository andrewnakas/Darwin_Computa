#!/bin/bash
# build-fhcli.sh — M37: build fhcli, a normal Objective-C Foundation program
# exercising NSFileHandle descriptor-level file I/O (write/read/seek), and
# stage it at the guest /usr/bin/fhcli.
#
...
# 2) Install at the guest path /usr/bin/fhcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/fhcli"
    chmod +x "$rootdir/usr/bin/fhcli"
    echo "installed: $rootdir/usr/bin/fhcli"
}
...
echo ""
echo "=== fhcli installed at guest /usr/bin/fhcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/fhcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-kvocli.sh — M58: build kvocli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/kvocli.
#
...
# 2) Install at the guest path /usr/bin/kvocli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/kvocli"
    chmod +x "$rootdir/usr/bin/kvocli"
    echo "installed: $rootdir/usr/bin/kvocli"
}
...
echo ""
echo "=== kvocli installed at guest /usr/bin/kvocli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/kvocli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

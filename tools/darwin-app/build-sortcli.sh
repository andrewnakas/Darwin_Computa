#!/bin/bash
# build-sortcli.sh — M58: build sortcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/sortcli.
#
...
# 2) Install at the guest path /usr/bin/sortcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/sortcli"
    chmod +x "$rootdir/usr/bin/sortcli"
    echo "installed: $rootdir/usr/bin/sortcli"
}
...
echo ""
echo "=== sortcli installed at guest /usr/bin/sortcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/sortcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

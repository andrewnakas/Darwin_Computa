#!/bin/bash
# build-strcmpcli.sh — M58: build strcmpcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/strcmpcli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/strcmpcli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/strcmpcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/strcmpcli"
    chmod +x "$rootdir/usr/bin/strcmpcli"
    echo "installed: $rootdir/usr/bin/strcmpcli"
}
...
echo ""
echo "=== strcmpcli installed at guest /usr/bin/strcmpcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/strcmpcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

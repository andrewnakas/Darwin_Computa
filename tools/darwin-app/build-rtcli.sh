#!/bin/bash
# build-rtcli.sh — M58: build rtcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/rtcli.
#
...
# 2) Install at the guest path /usr/bin/rtcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/rtcli"
    chmod +x "$rootdir/usr/bin/rtcli"
    echo "installed: $rootdir/usr/bin/rtcli"
}
...
echo ""
echo "=== rtcli installed at guest /usr/bin/rtcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/rtcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

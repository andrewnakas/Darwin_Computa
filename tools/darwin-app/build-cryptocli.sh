#!/bin/bash
# build-cryptocli.sh — M20: build cryptocli, a cryptographic digest + HMAC probe on
# the MODERN staged libcrypto (OpenSSL), and stage it at /usr/bin/cryptocli.
#
...
install_bin() {
    ...
    cp "$BIN" "$rootdir/usr/bin/cryptocli"
    chmod +x "$rootdir/usr/bin/cryptocli"
    echo "installed: $rootdir/usr/bin/cryptocli"
}
...
echo ""
echo "=== cryptocli installed at guest /usr/bin/cryptocli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/cryptocli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

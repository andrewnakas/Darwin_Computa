#!/bin/bash
# build-scan2cli.sh — M52: build scan2cli, a normal Objective-C Foundation program
# exercising deeper NSScanner scanning (float/hex/longlong/charset), and
# stage it at the guest /usr/bin/scan2cli.
#
...
# 2) Install at the guest path /usr/bin/scan2cli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/scan2cli"
    chmod +x "$rootdir/usr/bin/scan2cli"
    echo "installed: $rootdir/usr/bin/scan2cli"
}
...
echo ""
echo "=== scan2cli installed at guest /usr/bin/scan2cli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/scan2cli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

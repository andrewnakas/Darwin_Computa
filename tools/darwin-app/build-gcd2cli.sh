#!/bin/bash
# build-gcd2cli.sh — M56: build gcd2cli, a normal Objective-C Foundation program
# exercising GCD coordination primitives (group/after/barrier), and
# stage it at the guest /usr/bin/gcd2cli.
#
...
# 2) Install at the guest path /usr/bin/gcd2cli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/gcd2cli"
    chmod +x "$rootdir/usr/bin/gcd2cli"
    echo "installed: $rootdir/usr/bin/gcd2cli"
}
...
echo ""
echo "=== gcd2cli installed at guest /usr/bin/gcd2cli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/gcd2cli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

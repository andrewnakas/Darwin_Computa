#!/bin/bash
# build-numf2cli.sh — M58: build numf2cli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/numf2cli.
#
...
# 2) Install at the guest path /usr/bin/numf2cli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/numf2cli"
    chmod +x "$rootdir/usr/bin/numf2cli"
    echo "installed: $rootdir/usr/bin/numf2cli"
}
...
echo ""
echo "=== numf2cli installed at guest /usr/bin/numf2cli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/numf2cli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

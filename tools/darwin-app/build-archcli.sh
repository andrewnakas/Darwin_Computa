#!/bin/bash
# build-archcli.sh — M25: build archcli, a normal Objective-C Foundation program
# exercising NSKeyedArchiver object serialization + NSPropertyListSerialization plist round-trip, and
# stage it at the guest /usr/bin/archcli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/archcli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/archcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/archcli"
    chmod +x "$rootdir/usr/bin/archcli"
    echo "installed: $rootdir/usr/bin/archcli"
}
...
echo ""
echo "=== archcli installed at guest /usr/bin/archcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/archcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

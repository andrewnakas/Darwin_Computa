#!/bin/bash
# build-csvcli.sh — M39: build csvcli, a normal Objective-C Foundation program
# exercising a CLI synthesis: NSFileHandle read -> NSString split -> NSDecimalNumber sum -> JSON, and
# stage it at the guest /usr/bin/csvcli.
#
...
# 2) Install at the guest path /usr/bin/csvcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/csvcli"
    chmod +x "$rootdir/usr/bin/csvcli"
    echo "installed: $rootdir/usr/bin/csvcli"
}
...
echo ""
echo "=== csvcli installed at guest /usr/bin/csvcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/csvcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

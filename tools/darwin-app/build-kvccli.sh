#!/bin/bash
# build-kvccli.sh — M58: build kvccli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/kvccli.
#
...
# 2) Install at the guest path /usr/bin/kvccli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/kvccli"
    chmod +x "$rootdir/usr/bin/kvccli"
    echo "installed: $rootdir/usr/bin/kvccli"
}
...
echo ""
echo "=== kvccli installed at guest /usr/bin/kvccli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/kvccli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-maptblcli.sh — M58: build maptblcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/maptblcli.
#
...
# 2) Install at the guest path /usr/bin/maptblcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/maptblcli"
    chmod +x "$rootdir/usr/bin/maptblcli"
    echo "installed: $rootdir/usr/bin/maptblcli"
}
...
echo ""
echo "=== maptblcli installed at guest /usr/bin/maptblcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/maptblcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

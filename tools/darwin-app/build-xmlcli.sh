#!/bin/bash
# build-xmlcli.sh — M3: build xmlcli, a NORMAL Objective-C
# Foundation program (compiler-emitted objc_msgSend, NOT a hand-rolled shim like
# akapp.c), and stage it into the guest filesystem at /usr/bin/xmlcli.
#
# Cross-compiles xmlcli.m on the macOS host (Apple clang, x86_64) against
...
#   bash tools/run_darling_cli.sh /usr/bin/xmlcli
# or over shellspawn:
...
# 2) Install at the guest path /usr/bin/xmlcli in BOTH roots (live overlay
#    + dist stage), same as build-akapp.sh stages the bundle.
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/xmlcli"
    chmod +x "$rootdir/usr/bin/xmlcli"
    echo "installed: $rootdir/usr/bin/xmlcli"
}
...
echo ""
echo "=== xmlcli installed at guest /usr/bin/xmlcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/xmlcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-parcli.sh — M57: CONCURRENCY SYNTHESIS (JSON -> dispatch_apply parallel map +
# barrier accumulate -> HMAC-SHA256). Stage at /usr/bin/parcli. Links Foundation +
...
clang -target x86_64-apple-macos10.15 -fno-objc-arc -fobjc-runtime=macosx-10.15 \
  ...
  "$HERE/parcli.m" "$LIBSYSTEM" "$LIBOBJC" "$FOUNDATION" "$COREFOUNDATION" "$LIBCRYPTO"
echo "built binary: $BIN"; file "$BIN"
install_bin() { local r="$1"; install -d "$r/usr/bin"; cp "$BIN" "$r/usr/bin/parcli"; chmod +x "$r/usr/bin/parcli"; echo "installed: $r/usr/bin/parcli"; }
...
echo ""; echo "=== parcli installed at guest /usr/bin/parcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/parcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

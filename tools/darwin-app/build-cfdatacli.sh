#!/bin/bash
# build-cfdatacli.sh — M46: build cfdatacli, a normal Objective-C Foundation program
# exercising CFData + CFString operations via the CoreFoundation C API, and
# stage it at the guest /usr/bin/cfdatacli.
#
...
# 2) Install at the guest path /usr/bin/cfdatacli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/cfdatacli"
    chmod +x "$rootdir/usr/bin/cfdatacli"
    echo "installed: $rootdir/usr/bin/cfdatacli"
}
...
echo ""
echo "=== cfdatacli installed at guest /usr/bin/cfdatacli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/cfdatacli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

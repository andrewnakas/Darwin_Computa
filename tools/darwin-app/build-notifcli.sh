#!/bin/bash
# build-notifcli.sh — M58: build notifcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/notifcli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/notifcli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/notifcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/notifcli"
    chmod +x "$rootdir/usr/bin/notifcli"
    echo "installed: $rootdir/usr/bin/notifcli"
}
...
echo ""
echo "=== notifcli installed at guest /usr/bin/notifcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/notifcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

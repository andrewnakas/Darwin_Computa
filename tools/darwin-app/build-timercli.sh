#!/bin/bash
# build-timercli.sh — M36: build timercli, a normal Objective-C Foundation program
# exercising NSTimer + NSRunLoop event-loop machinery, and
# stage it at the guest /usr/bin/timercli.
#
...
# 2) Install at the guest path /usr/bin/timercli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/timercli"
    chmod +x "$rootdir/usr/bin/timercli"
    echo "installed: $rootdir/usr/bin/timercli"
}
...
echo ""
echo "=== timercli installed at guest /usr/bin/timercli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/timercli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

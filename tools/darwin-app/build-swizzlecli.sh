#!/bin/bash
# build-swizzlecli.sh — M58: build swizzlecli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/swizzlecli.
#
...
# 2) Install at the guest path /usr/bin/swizzlecli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/swizzlecli"
    chmod +x "$rootdir/usr/bin/swizzlecli"
    echo "installed: $rootdir/usr/bin/swizzlecli"
}
...
echo ""
echo "=== swizzlecli installed at guest /usr/bin/swizzlecli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/swizzlecli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

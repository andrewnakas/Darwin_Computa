#!/bin/bash
# build-b64modcli.sh — M58: build b64modcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/b64modcli.
#
...
# 2) Install at the guest path /usr/bin/b64modcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/b64modcli"
    chmod +x "$rootdir/usr/bin/b64modcli"
    echo "installed: $rootdir/usr/bin/b64modcli"
}
...
echo ""
echo "=== b64modcli installed at guest /usr/bin/b64modcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/b64modcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

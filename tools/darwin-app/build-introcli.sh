#!/bin/bash
# build-introcli.sh — M58: build introcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/introcli.
#
...
# 2) Install at the guest path /usr/bin/introcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/introcli"
    chmod +x "$rootdir/usr/bin/introcli"
    echo "installed: $rootdir/usr/bin/introcli"
}
...
echo ""
echo "=== introcli installed at guest /usr/bin/introcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/introcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

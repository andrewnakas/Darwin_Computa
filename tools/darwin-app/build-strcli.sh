#!/bin/bash
# build-strcli.sh — M29: build strcli, a normal Objective-C Foundation program
# exercising everyday NSString text processing (split/join/trim/case/search/replace), and
# stage it at the guest /usr/bin/strcli.
#
...
# 2) Install at the guest path /usr/bin/strcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/strcli"
    chmod +x "$rootdir/usr/bin/strcli"
    echo "installed: $rootdir/usr/bin/strcli"
}
...
echo ""
echo "=== strcli installed at guest /usr/bin/strcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/strcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

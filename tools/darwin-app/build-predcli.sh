#!/bin/bash
# build-predcli.sh — M24: build predcli, a normal Objective-C Foundation program
# exercising NSPredicate filtering + NSSortDescriptor sorting + NSExpression eval, and
# stage it at the guest /usr/bin/predcli.
#
...
# 2) Install at the guest path /usr/bin/predcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/predcli"
    chmod +x "$rootdir/usr/bin/predcli"
    echo "installed: $rootdir/usr/bin/predcli"
}
...
echo ""
echo "=== predcli installed at guest /usr/bin/predcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/predcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-enccli.sh — M38: build enccli, a normal Objective-C Foundation program
# exercising NSString text encoding conversion (UTF-8/UTF-16) + encoded file I/O, and
# stage it at the guest /usr/bin/enccli.
#
...
# 2) Install at the guest path /usr/bin/enccli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/enccli"
    chmod +x "$rootdir/usr/bin/enccli"
    echo "installed: $rootdir/usr/bin/enccli"
}
...
echo ""
echo "=== enccli installed at guest /usr/bin/enccli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/enccli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

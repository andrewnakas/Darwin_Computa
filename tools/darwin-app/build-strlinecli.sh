#!/bin/bash
# build-strlinecli.sh — M58: build strlinecli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/strlinecli.
#
...
# 2) Install at the guest path /usr/bin/strlinecli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/strlinecli"
    chmod +x "$rootdir/usr/bin/strlinecli"
    echo "installed: $rootdir/usr/bin/strlinecli"
}
...
echo ""
echo "=== strlinecli installed at guest /usr/bin/strlinecli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/strlinecli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

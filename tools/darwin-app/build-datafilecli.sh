#!/bin/bash
# build-datafilecli.sh — M58: build datafilecli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/datafilecli.
#
...
# 2) Install at the guest path /usr/bin/datafilecli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/datafilecli"
    chmod +x "$rootdir/usr/bin/datafilecli"
    echo "installed: $rootdir/usr/bin/datafilecli"
}
...
echo ""
echo "=== datafilecli installed at guest /usr/bin/datafilecli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/datafilecli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

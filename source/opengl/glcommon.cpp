...
U32 int99CallbackSize;
U32 lastGlCallTime;
// x64-guest build: the legacy 32-bit GL int99 dispatch table is replaced by
// gl64bridge, so there is no callback table to populate. Keep the same
// signature startupArgs.cpp:apply() calls (gl_init(this->glExt)) so the clean
// x64 build links; the argument is unused here.
void gl_init(BString allowExtensions) {

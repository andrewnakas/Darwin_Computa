#!/bin/bash
# build-jsoncli.sh — M3: build jsoncli, a NORMAL Objective-C
# Foundation program (compiler-emitted objc_msgSend, NOT a hand-rolled shim like
# akapp.c), and stage it into the guest filesystem at /usr/bin/jsoncli.
#
# Cross-compiles jsoncli.m on the macOS host (Apple clang, x86_64) against
...
#   bash tools/run_darling_cli.sh /usr/bin/jsoncli
# or over shellspawn:
...
# 2) Install at the guest path /usr/bin/jsoncli in BOTH roots (live overlay
#    + dist stage), same as build-akapp.sh stages the bundle.
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/jsoncli"
    chmod +x "$rootdir/usr/bin/jsoncli"
    echo "installed: $rootdir/usr/bin/jsoncli"
}
...
echo ""
echo "=== jsoncli installed at guest /usr/bin/jsoncli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/jsoncli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-idxcli.sh — M48: build idxcli, a normal Objective-C Foundation program
# exercising NSIndexSet / NSMutableIndexSet index-set operations, and
# stage it at the guest /usr/bin/idxcli.
#
...
# 2) Install at the guest path /usr/bin/idxcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/idxcli"
    chmod +x "$rootdir/usr/bin/idxcli"
    echo "installed: $rootdir/usr/bin/idxcli"
}
...
echo ""
echo "=== idxcli installed at guest /usr/bin/idxcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/idxcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-opqcli.sh — M61: build opqcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/opqcli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/opqcli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/opqcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/opqcli"
    chmod +x "$rootdir/usr/bin/opqcli"
    echo "installed: $rootdir/usr/bin/opqcli"
}
...
echo ""
echo "=== opqcli installed at guest /usr/bin/opqcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/opqcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

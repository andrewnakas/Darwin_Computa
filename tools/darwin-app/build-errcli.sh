#!/bin/bash
# build-errcli.sh — M40: build errcli, a normal Objective-C Foundation program
# exercising NSError out-parameter propagation + ObjC @try/@catch/@throw, and
# stage it at the guest /usr/bin/errcli.
#
...
#   BW64_SHELLSPAWN=/usr/bin/errcli bash tools/run_darling_cli.sh /usr/bin/darlingserver
set -euo pipefail
...
# 2) Install at the guest path /usr/bin/errcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/errcli"
    chmod +x "$rootdir/usr/bin/errcli"
    echo "installed: $rootdir/usr/bin/errcli"
}
...
echo ""
echo "=== errcli installed at guest /usr/bin/errcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/errcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

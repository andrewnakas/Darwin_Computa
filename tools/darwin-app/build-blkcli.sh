#!/bin/bash
# build-blkcli.sh — M54: build blkcli, a normal Objective-C Foundation program
# exercising Obj-C blocks through Foundation block-based APIs, and
# stage it at the guest /usr/bin/blkcli.
#
...
# 2) Install at the guest path /usr/bin/blkcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/blkcli"
    chmod +x "$rootdir/usr/bin/blkcli"
    echo "installed: $rootdir/usr/bin/blkcli"
}
...
echo ""
echo "=== blkcli installed at guest /usr/bin/blkcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/blkcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

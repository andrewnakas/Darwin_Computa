#!/bin/bash
# build-proccli.sh — M35: build proccli, a normal Objective-C Foundation program
# exercising NSProcessInfo process + environment introspection, and
# stage it at the guest /usr/bin/proccli.
#
...
# 2) Install at the guest path /usr/bin/proccli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/proccli"
    chmod +x "$rootdir/usr/bin/proccli"
    echo "installed: $rootdir/usr/bin/proccli"
}
...
echo ""
echo "=== proccli installed at guest /usr/bin/proccli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/proccli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

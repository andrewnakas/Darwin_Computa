#!/bin/bash
# build-numcli.sh — M28: build numcli, a normal Objective-C Foundation program
# exercising NSNumberFormatter locale-aware number formatting + parsing, and
# stage it at the guest /usr/bin/numcli.
#
...
# 2) Install at the guest path /usr/bin/numcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/numcli"
    chmod +x "$rootdir/usr/bin/numcli"
    echo "installed: $rootdir/usr/bin/numcli"
}
...
echo ""
echo "=== numcli installed at guest /usr/bin/numcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/numcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

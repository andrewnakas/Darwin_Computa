...
const char* c_getTranslation(Msg msg, bool useDefaultIfMissing) {
    ...
    case Msg::OPTIONSVIEW_SAVE_FOLDER_HELP:
        return "This is the location on your computer where the games and apps will be installed.  If you already installed some games or apps, and you change this, you will no longer see them.  It is fine to delete this folder if you want to start over.";
    ...
    case Msg::HELPVIEW_ABOUT_LABEL:
        return "## What is Boxedwine?\n[Boxedwine](https://github.com/danoon2/Boxedwine) is a multi-platform app for running 16-bit and 32-bit Windows programs.  It does this by emulating the CPU and running [Wine](https://www.winehq.org/).  Wine is the program that emulates all of the Windows API calls.  The goal of Boxedwine is to be able to run everything that Wine can.  If you want to know if your app or game will run in Boxedwine or get hints how to run it, you should start by looking at the [Wine Application Database (AppDB)](https://appdb.winehq.org/)\n## Supported Platforms:\n* Windows x86/x64/Arm64 (Vista or later)\n* Mac OSX\n* Linux x86/x64/Arm64\n* Web Browsers using WASM\n## Current Version: 26R1\n* Rewrote JIT CPU emulation.  It is now 50%-100% faster depending on platform and uses 20% less memory.\n*Wine 11 now supported.\n*OpenGL backend D3D12 added to Windows ARM64\n## Older Version: 25R1\n* Added support for Wine 6, 9 and 10.\n* Bug fixes for all CPU cores. Cinebench 11.5, iexplore and dot net using mono works.\n* OpenGL fix for vertexes.  Now Alice works.\n* Reduced memory usage for x64 and Armv8 CPU cores.\n* Normal and JIT cpu cores are now multi-threaded.\n## Some Known Issues:\n* Dot Net 8 does not work.\n* iexplore doesn't work with Wine 10.\n* Multi-threaded WASM/Emscripten build has some sound stuttering.\n* On Linux some OpengGL driver don't handle old OpenGL features, games like Thief may have issues.\n* There are some random crashes, especially during start up and shutdown of some apps/games, sometimes trying again will work.";
    ...
    case Msg::COMMON_FILESYSTEM_VERSION_HELP:
        return "If you are not sure, then just use the default value which should be the most recent Wine version.  This value may cause issues if changed later.";

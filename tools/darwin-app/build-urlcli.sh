#!/bin/bash
# build-urlcli.sh — M27: build urlcli, a normal Objective-C Foundation program
# exercising NSString path manipulation + NSURL parsing/composition, and
# stage it at the guest /usr/bin/urlcli.
#
...
# 2) Install at the guest path /usr/bin/urlcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/urlcli"
    chmod +x "$rootdir/usr/bin/urlcli"
    echo "installed: $rootdir/usr/bin/urlcli"
}
...
echo ""
echo "=== urlcli installed at guest /usr/bin/urlcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/urlcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-bytecli.sh — M51: build bytecli, a normal Objective-C Foundation program
# exercising NSByteCountFormatter human-readable byte-count formatting, and
# stage it at the guest /usr/bin/bytecli.
#
...
# 2) Install at the guest path /usr/bin/bytecli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/bytecli"
    chmod +x "$rootdir/usr/bin/bytecli"
    echo "installed: $rootdir/usr/bin/bytecli"
}
...
echo ""
echo "=== bytecli installed at guest /usr/bin/bytecli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/bytecli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

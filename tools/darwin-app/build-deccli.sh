#!/bin/bash
# build-deccli.sh — M26: build deccli, a normal Objective-C Foundation program
# exercising NSDecimalNumber exact decimal arithmetic + NSCountedSet multiset counting, and
# stage it at the guest /usr/bin/deccli.
#
...
# 2) Install at the guest path /usr/bin/deccli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/deccli"
    chmod +x "$rootdir/usr/bin/deccli"
    echo "installed: $rootdir/usr/bin/deccli"
}
...
echo ""
echo "=== deccli installed at guest /usr/bin/deccli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/deccli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-arrcli.sh — M58: build arrcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/arrcli.
#
...
# 2) Install at the guest path /usr/bin/arrcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/arrcli"
    chmod +x "$rootdir/usr/bin/arrcli"
    echo "installed: $rootdir/usr/bin/arrcli"
}
...
echo ""
echo "=== arrcli installed at guest /usr/bin/arrcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/arrcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

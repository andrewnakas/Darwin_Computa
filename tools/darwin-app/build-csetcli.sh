#!/bin/bash
# build-csetcli.sh — M43: build csetcli, a normal Objective-C Foundation program
# exercising NSCharacterSet predefined classes + membership/inversion, and
# stage it at the guest /usr/bin/csetcli.
#
...
# 2) Install at the guest path /usr/bin/csetcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/csetcli"
    chmod +x "$rootdir/usr/bin/csetcli"
    echo "installed: $rootdir/usr/bin/csetcli"
}
...
echo ""
echo "=== csetcli installed at guest /usr/bin/csetcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/csetcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-cachecli.sh — M58: build cachecli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/cachecli.
#
...
# 2) Install at the guest path /usr/bin/cachecli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/cachecli"
    chmod +x "$rootdir/usr/bin/cachecli"
    echo "installed: $rootdir/usr/bin/cachecli"
}
...
echo ""
echo "=== cachecli installed at guest /usr/bin/cachecli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/cachecli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-valcli.sh — M49: build valcli, a normal Objective-C Foundation program
# exercising NSValue boxing of C structs (NSRange/NSPoint/NSSize), and
# stage it at the guest /usr/bin/valcli.
#
...
# 2) Install at the guest path /usr/bin/valcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/valcli"
    chmod +x "$rootdir/usr/bin/valcli"
    echo "installed: $rootdir/usr/bin/valcli"
}
...
echo ""
echo "=== valcli installed at guest /usr/bin/valcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/valcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

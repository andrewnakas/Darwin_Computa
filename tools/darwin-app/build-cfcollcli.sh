#!/bin/bash
# build-cfcollcli.sh — M45: build cfcollcli, a normal Objective-C Foundation program
# exercising CFArray + CFDictionary + CFNumber via the CoreFoundation C API, and
# stage it at the guest /usr/bin/cfcollcli.
#
...
# 2) Install at the guest path /usr/bin/cfcollcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/cfcollcli"
    chmod +x "$rootdir/usr/bin/cfcollcli"
    echo "installed: $rootdir/usr/bin/cfcollcli"
}
...
echo ""
echo "=== cfcollcli installed at guest /usr/bin/cfcollcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/cfcollcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

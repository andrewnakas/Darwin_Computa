#!/bin/bash
# build-attrcli.sh — M58: build attrcli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/attrcli.
#
...
# 2) Install at the guest path /usr/bin/attrcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/attrcli"
    chmod +x "$rootdir/usr/bin/attrcli"
    echo "installed: $rootdir/usr/bin/attrcli"
}
...
echo ""
echo "=== attrcli installed at guest /usr/bin/attrcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/attrcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

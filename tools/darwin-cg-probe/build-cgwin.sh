#!/bin/bash
# Build cgwin — a minimal CoreGraphics client Mach-O for the Darwin guest (S29).
#
...
# See cgwin.c header for why. Mirrors tools/darwin-x11-probe/build.sh (xprobe).
set -euo pipefail
...
#   the C runtime; cgwin has no static initializers of its own so jumping

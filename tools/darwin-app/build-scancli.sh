#!/bin/bash
# build-scancli.sh — M23: build scancli, a normal Objective-C Foundation program
# exercising NSScanner string tokenizing + NSUUID generation/round-trip, and
# stage it at the guest /usr/bin/scancli.
#
...
# 2) Install at the guest path /usr/bin/scancli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/scancli"
    chmod +x "$rootdir/usr/bin/scancli"
    echo "installed: $rootdir/usr/bin/scancli"
}
...
echo ""
echo "=== scancli installed at guest /usr/bin/scancli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/scancli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-fmcli.sh — M3: build fmcli, a NORMAL Objective-C
# Foundation program (compiler-emitted objc_msgSend, NOT a hand-rolled shim like
# akapp.c), and stage it into the guest filesystem at /usr/bin/fmcli.
#
# Cross-compiles fmcli.m on the macOS host (Apple clang, x86_64) against
...
#   bash tools/run_darling_cli.sh /usr/bin/fmcli
# or over shellspawn:
...
# 2) Install at the guest path /usr/bin/fmcli in BOTH roots (live overlay
#    + dist stage), same as build-akapp.sh stages the bundle.
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/fmcli"
    chmod +x "$rootdir/usr/bin/fmcli"
    echo "installed: $rootdir/usr/bin/fmcli"
}
...
echo ""
echo "=== fmcli installed at guest /usr/bin/fmcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/fmcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

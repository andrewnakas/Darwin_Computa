#!/bin/bash
# build-dynclscli.sh — M58: build dynclscli, a normal Objective-C Foundation program
# exercising Key-Value Coding (KVC) + block-based NSPredicate, and
# stage it at the guest /usr/bin/dynclscli.
#
...
# 2) Install at the guest path /usr/bin/dynclscli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/dynclscli"
    chmod +x "$rootdir/usr/bin/dynclscli"
    echo "installed: $rootdir/usr/bin/dynclscli"
}
...
echo ""
echo "=== dynclscli installed at guest /usr/bin/dynclscli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/dynclscli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

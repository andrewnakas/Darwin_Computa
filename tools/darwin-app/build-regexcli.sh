#!/bin/bash
# build-regexcli.sh — M3: build regexcli, a NORMAL Objective-C
# Foundation program (compiler-emitted objc_msgSend, NOT a hand-rolled shim like
# akapp.c), and stage it into the guest filesystem at /usr/bin/regexcli.
#
# Cross-compiles regexcli.m on the macOS host (Apple clang, x86_64) against
...
#   bash tools/run_darling_cli.sh /usr/bin/regexcli
# or over shellspawn:
...
# 2) Install at the guest path /usr/bin/regexcli in BOTH roots (live overlay
#    + dist stage), same as build-akapp.sh stages the bundle.
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/regexcli"
    chmod +x "$rootdir/usr/bin/regexcli"
    echo "installed: $rootdir/usr/bin/regexcli"
}
...
echo ""
echo "=== regexcli installed at guest /usr/bin/regexcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/regexcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-deccli2.sh — M53: build deccli2, a normal Objective-C Foundation program
# exercising NSDecimalNumber rounding/division/comparison via NSDecimalNumberHandler, and
# stage it at the guest /usr/bin/deccli2.
#
...
# 2) Install at the guest path /usr/bin/deccli2 in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/deccli2"
    chmod +x "$rootdir/usr/bin/deccli2"
    echo "installed: $rootdir/usr/bin/deccli2"
}
...
echo ""
echo "=== deccli2 installed at guest /usr/bin/deccli2 ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/deccli2 bash tools/run_darling_cli.sh /usr/bin/darlingserver"

#!/bin/bash
# build-cfuuidcli.sh — M44: build cfuuidcli, a normal Objective-C Foundation program
# exercising CFUUID + CFString via the CoreFoundation C API (CF C-layer), and
# stage it at the guest /usr/bin/cfuuidcli.
#
...
# 2) Install at the guest path /usr/bin/cfuuidcli in BOTH roots (live overlay +
#    dist stage).
install_bin() {
    local rootdir="$1"
    install -d "$rootdir/usr/bin"
    cp "$BIN" "$rootdir/usr/bin/cfuuidcli"
    chmod +x "$rootdir/usr/bin/cfuuidcli"
    echo "installed: $rootdir/usr/bin/cfuuidcli"
}
...
echo ""
echo "=== cfuuidcli installed at guest /usr/bin/cfuuidcli ==="
echo "Run:  BW64_SHELLSPAWN=/usr/bin/cfuuidcli bash tools/run_darling_cli.sh /usr/bin/darlingserver"
